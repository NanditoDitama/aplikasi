...
public class EditReportActivity extends AppCompatActivity {
    ...
    private void updateReport() {
        ...
        if (selectedImageUri != null) {
            ...
            createAndSendEditRequest(title, description, amount, date, currentReport.getImageUrl());
        }
    }
    ...
    private void uploadImage(String title, String description, double amount, Timestamp date) {
        try {
            ...
            String newImageName = "report_" + UUID.randomUUID().toString() + ".jpg";
            StorageReference storageRef = storage.getReference().child("report_images/" + newImageName);
                    ...
                    .addOnSuccessListener(taskSnapshot -> {
                        storageRef.getDownloadUrl().addOnSuccessListener(uri -> {
                            ...
                            if (currentReport != null && currentReport.getImageUrl() != null
                                    && !currentReport.getImageUrl().isEmpty()) {
                                deleteOldImage(currentReport.getImageUrl());
                            }
                            createAndSendEditRequest(title, description, amount, date, imageUrl);
                        });
                    })
    ...
    private void deleteOldImage(String oldImageUrl) {
        ...
        oldImageRef.delete().addOnFailureListener(e -> {
            Log.e("EditReportActivity", "Failed to delete old image: " + e.getMessage());
        });
    }

    private void createAndSendEditRequest(String title, String description, double amount, Timestamp date, String imageUrl) {
        ...
        Report updatedReport = new Report(
                ...
                imageUrl,
                currentUser.getUid(),
                currentReport.getRecipientId()
        );

        EditRequest editRequest = new EditRequest();
        editRequest.setId(UUID.randomUUID().toString());
        editRequest.setReportId(currentReport.getId());
        editRequest.setUserId(currentUser.getUid());
        editRequest.setUserEmail(currentUser.getEmail());
        editRequest.setUserName(currentUser.getDisplayName());
        editRequest.setOriginalReport(currentReport);
        editRequest.setUpdatedReport(updatedReport);
        editRequest.setRequestDate(new Date());
        editRequest.setStatus("pending");

        db.collection("editRequests").document(editRequest.getId())
                .set(editRequest)
                .addOnSuccessListener(aVoid -> {
                    progressBar.setVisibility(View.GONE);
                    buttonUpdateReport.setEnabled(true);
                    Toast.makeText(this, "Edit request sent for approval", Toast.LENGTH_SHORT).show();
                    finish();
                })
                .addOnFailureListener(e -> {
                    progressBar.setVisibility(View.GONE);
